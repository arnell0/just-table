...
function Table(props) {
  ...
  React.useEffect(() => {
    ...
    if (_settings.autoGenerateColumns === true) {
      ...
      _columns.forEach((column, index) => {
        _columns[index] = column
          .split("_")
          .join(" ") // replace underscore with space
          .replace(/([A-Z])/g, " $1") // split by capital letter
          .split("-")
          .join(" ") // replace dash with space
          .split(" ")
    ...
    if (_settings.pagination === true) {
      _values = _values.slice(
        (page - 1) * _settings.paginationSize,
        page * _settings.paginationSize
      );
    }
      ...
      >
        <tr>
          ...
          <th>
            ...
            >
              <div className="icon expand" onClick={handleColumnCreate}>
                {icons_plus}
              </div>
            </div>
          </th>
  ...
  const handleRowClick = (row) => {
    ...
    settings.onRowClick(dataRow);
  };

  const handleRowCreate = () => {
    settings.onRowCreate();
  };
      ...
      <tbody>
        ...
        <tr>
          <td>
            ...
            >
              <div className="icon expand" onClick={handleRowCreate}>
                {icons_plus}
              </div>
            </div>
          </td>
          ...
          {columns.length > 0 &&
            columns.map((column, index) => <td key={index}></td>)}
        </tr>
      </tbody>
    );
  };
  ...
  const handlePagination = (newPage, _values, newPaginationSize) => {
    ...
    _values = _values.slice(
      (newPage - 1) * paginationSize,
      newPage * paginationSize
    );
      ...
      {settings.search === true && (
        <tr>
          <td colSpan={columns.length + 1}>
            ...
            >
              <SearchBox
                values={extractBaseValues(props.data)}
